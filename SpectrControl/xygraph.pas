...
uses Windows , {Messages,} SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
    {StdCtrls, Spin,} ExtCtrls, Clipbrd{, AppEvnts, xycommon},specialtypes;
...
function XYMouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer):TPointR;
function XYMouseMove(Shift: TShiftState;  X, Y: Integer):TPointR;
function XYMouseUp(Button: TMouseButton; var Shift: TShiftState; X, Y: Integer):TPointR;
...
function showcoord(x,y:integer;show:boolean):TPointR; {XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}
var s,ss,s3d : string;
    l,n,n2,r : integer;
    xw,yw,h : single;
    xwd : double;
     ...
     begin
       with as_[0] do
         if crossrel then
           ...
           else begin n := min(ndec,ninc); xw := xwd; s := Format('%.2f',[xw]); point.X:=xw; end;
       with as_[coordas] do
         if crossrel then
           ...
           begin n := min(ndec,ninc); ss := Format('%.2f',[yw]); point.Y:=yw; Result:=point; end;
         if rul3d then
            begin
...
function cross(x,y:integer;show:boolean):TPointR; {XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}
 {tekent lineaal}
begin
   crossx := x; crossy := y;
   tekencross(x,y); if coordok then Result:=showcoord(x,y,show);
end;
...
function XYMouseMove(Shift: TShiftState;  X, Y: Integer):TPointR; {XXXXXXXXXXXXXXXXXXX}
begin
  if (ssleft in shift) then
    begin
      if crosson then
         begin snap(x,y); if (x=crossx) and (y=crossy) then exit;
            cross(crossx,crossy,false); fillexport(x,y,xyexportd); result := cross(x,y,true);
            xdataold := xdata; ydataold := ydata;
         end;
...
begin
  if (button=mbleft) then
   if (track>0) and (ssshift in shift) then 
    ...
    begin
      xdataold := xdata; ydataold := ydata;
      if crosson then  begin Result:= cross(crossx,crossy,false);

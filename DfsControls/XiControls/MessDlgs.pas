...
{     Turkish translation by Egemen ЮEN                                  }
...
function InputQueryI(const ACaption, APrompt: string;
  var Value: string; Lang: TLangType;Password:boolean): Boolean;

function InputQuery(const ACaption, APrompt: string;
  var Value: string): Boolean;

function InputBoxI(const ACaption, APrompt, ADefault: string; Lang: TLangType;pass:boolean):
  string;
...
uses Types, XiProgressBar, XiPanel, XiButton;
...
begin
  ...
  try
    ...
    Caption := ACaption;
    ClientWidth := MulDiv(220, DialogUnits.X, 4);
    ...
    begin
      ...
      Width := MulDiv(200, DialogUnits.X, 4);
      MaxLength := 255;
      Text := Value;
      SelectAll;
      if Password = true then edit.PasswordChar:='*';
    end;
    ...
    if MsgOptions.UseCustomButtons then
      ...
      begin
        ...
        ModalResult := mrOk;
        Default := True;
        SetBounds(MulDiv(55, DialogUnits.X, 4), ButtonTop, ButtonWidth,
          ButtonHeight);
        ColorScheme :=
          XiButton.TColorScheme(MsgOptions.CustomButtonsColorScheme);
      end
    ...
    if MsgOptions.UseCustomButtons then
      ...
      begin
        ...
        Cancel := True;
        SetBounds(MulDiv(109, DialogUnits.X, 4), ButtonTop, ButtonWidth,
          ButtonHeight);
        ColorScheme :=
          XiButton.TColorScheme(MsgOptions.CustomButtonsColorScheme);
...
function InputQuery(const ACaption, APrompt: string;
  var Value: string): Boolean;
begin
  result := InputQueryI(ACaption, APrompt, VAlue, MsgOptions.DefLang,false);
end;
...
begin
  Result := ADefault;
  InputQueryI(ACaption, APrompt, Result, Lang,pass);
end;

function InputBox(const ACaption, APrompt, ADefault: string): string;
begin
  Result := InputBoxI(ACaption, APrompt, ADefault, MsgOptions.DefLang,false);
end;
...
begin
  CheckStatus(False);
  StatusForm := TMessageForm.CreateNew(Application.MainForm);
 Statusform.Position := poOwnerFormCenter;
